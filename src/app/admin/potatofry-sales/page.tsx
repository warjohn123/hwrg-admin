...
import PotatoFryReportDetails from '@/components/PotatoFryReportDetails';
...
export default function ReportsPage() {
  const [salesReports, setSalesReports] = useState<IPotatoFryReport[]>([]);
  ...
  async function getBranches() {
    const res = await fetchBranches(IAssignment.POTATO_FRY);
  ...
  const totalSales = salesReports.reduce((acc, report) => {
    return acc + getPotatoFryTotalSales(report.sales);
  }, 0);
  ...
  function getShortAndOver(report: IPotatoFryReport) {
            ...
            {salesReports.map((report) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  {getPotatoFryTotalSales(
                    report.sales as PotatoFrySales,
                  ).toLocaleString()}
                </td>
